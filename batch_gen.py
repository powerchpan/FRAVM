...
import numpy as np
import random
from data_msgcn.signals.disps import get_displacements
from data_msgcn.signals.rel_coords import get_relative_coordinates
...
class BatchGenerator(object):
    ...
    def next_batch(self, batch_size):
        ...
        for vid in batch:
            ...
            file_ptr = np.load(self.gt_path + vid, allow_pickle=True)
            # except ValueError:
            #     print('stop')
            classes = np.zeros(min(np.shape(features)[1], len(file_ptr)), dtype=int)
            for i in range(len(classes)):
                classes[i] = file_ptr[i]
