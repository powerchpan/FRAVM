...
def get_parameters_sts(keypoints, results):
   ...
   # single_sts_duration_arr = np.array(single_sts_duration)
    ...
    # duration of each sitting position, totally 4 - OK
    parameters_list.append(each_sitting_time)
    
    # duration of each sit-to-stand transition, totally 5
    parameters_list.append(each_sitstand_time)
    
    # duration of each stand-to-sit transition, totally 5
    parameters_list.append(each_standsit_time)
    
    # velocity of each sit-to-stand transition, totally 5
    parameters_list.append(sitstand_velocity)
    
    # velocity of each stand-to-sit transition, totally 5
    parameters_list.append(standsit_velocity)
    
    # velocity of each sit-to-stand-to-sit cycle, totally 5
    parameters_list.append(sts_velocity)
    
    # average velocity of five sit-to-stand transitions, totally 1
    parameters_list.append(mean_sitstand_velocity)
    
    # average velocity of five stand-to-sit transitions, totally 1 
    parameters_list.append(mean_standsit_velocity)
    
    # average velocity of whole sit-to-stand process, totally 1
    parameters_list.append(mean_sts_velocity)
    
    # maximum velocity of five sit-to-stand transitions, totally 1
    parameters_list.append(max_sitstand_velocity)
    
    # maximum velocity of five stand-to-sit transitions, totally 1 
    parameters_list.append(max_standsit_velocity)
    
    # maximum velocity of whole sit-to-stand process, totally 1
    parameters_list.append(max_sts_velocity)
    
    # minimum velocity of five sit-to-stand transitions, totally 1
    parameters_list.append(min_sitstand_velocity)
    
    # minimum velocity of five stand-to-sit transitions, totally 1
    parameters_list.append(min_standsit_velocity)
    
    # minimum velocity of whole sit-to-stand process, totally 1
    parameters_list.append(min_sts_velocity)
    
    # acceleration of each sit-to-stand transition, totally 5
    parameters_list.append(sitstand_accelaration)
    
    # acceleration of each stand-to-sit transition, totally 5
    parameters_list.append(standsit_accelaration)
    
    # acceleration of each sit-to-stand-to-sit cycle, totally 5
    parameters_list.append(each_sts_accelaration)
    
    # average duration of four sitting positions, totally 1
    parameters_list.append(mean_sitting_time)
    
    # average duration of four standing positions, totally 1
    parameters_list.append(mean_standing_time)
    
    # duration of whole sts process, totally 1
    parameters_list.append(single_sts_duration)
    
    
    # output length of each parameter for testing
    # 4
    print('len(each_sitting_time):', len(each_sitting_time))
    # 5
    print('len(each_sitstand_time):', len(each_sitstand_time))
    # 5		
    print('len(each_standsit_time):', len(each_standsit_time))
    # 5
    print('len(sitstand_velocity):', len(sitstand_velocity))
    # 5
    print('len(standsit_velocity):', len(standsit_velocity))
    # 5
    print('len(sts_velocity):', len(sts_velocity))
    # 1
    print('len(mean_sitstand_velocity): 1')
    # 1
    print('len(mean_standsit_velocity): 1')
    # 1
    print('len(mean_sts_velocity): 1')
    # 1
    print('len(max_sitstand_velocity): 1')
    # 1
    print('len(max_standsit_velocity): 1')
    # 1
    print('len(max_sts_velocity): 1')
    # 1
    print('len(min_sitstand_velocity): 1')
    # 1
    print('len(min_standsit_velocity): 1')
    # 1
    print('len(min_sts_velocity): 1')
    
    print('len(sitstand_accelaration):', len(sitstand_accelaration))
    
    print('len(standsit_accelaration):', len(standsit_accelaration))
    
    print('len(each_sts_accelaration):', len(each_sts_accelaration))
    # 1
    print('len(mean_sitting_time): 1')
    # 1
    print('len(mean_standing_time): 1')
    
    print('len(single_sts_duration):', len(single_sts_duration))
...
def main(args):
    ...
    vibe_fps = num_frames / (end - vibe_time)

    print(f'VIBE FPS: {vibe_fps:.2f}')
...
if __name__ == '__main__':
    ...
    temp_video = cv2.VideoCapture(video_file)
    get_feature_2.fps = temp_video.get(cv2.CAP_PROP_FPS)
    print('get_feature_2.fps in test.py:', get_feature_2.fps)
    temp_video.release()
   ...
   # min_index, keypoints, keypoints_max = choose_keypoints(standard_dir, all_keypoints)
    ...
    # output segmentation results to excel file.
    df_segmentation_results = pd.DataFrame(results)
    df_segmentation_results_writer = pd.ExcelWriter('segmentation-results.xlsx')
    # df_all.to_excel(writer, 'sheet_1', float_format='%f', index=False, header=None)
    df_segmentation_results.to_excel(df_segmentation_results_writer, 'sheet_1', float_format='%f', index=False, header=False)
    df_segmentation_results_writer.save()
    df_segmentation_results_writer.close()
    ...
    parameters2 = parameters.reshape(1, parameters.shape[0])
    print('all_time:', all_time)
    print('parameters2:')
    print(parameters2, '\n')
    print('parameters2.shape:', parameters2.shape)
    df = pd.DataFrame(parameters2)
    
    df.insert(loc=0, column='video-name', value=video_file)
    # print(df)
    print('----------------------------------------------------')

    # df_original = pd.DataFrame(pd.read_excel('data.xlsx', sheet_name='sheet_1', header=None))
    df_original = pd.DataFrame(pd.read_excel('data.xlsx', sheet_name='sheet_1'))
    ...
    df_all = pd.concat([df_original, df])
    print('df_all:')
    print(df_all, '\n')

    writer = pd.ExcelWriter('data.xlsx')
    # df_all.to_excel(writer, 'sheet_1', float_format='%f', index=False, header=None)
    df_all.to_excel(writer, 'sheet_1', float_format='%f', index=False, header=True)
    writer.save()
    writer.close()
