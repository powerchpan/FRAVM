...
from models_msgcn.ms_gcn import MultiStageModel
...
class Trainer:
    ...
    def predict(self, model_dir, results_dir, features_path, vid_list_file, epoch, actions_dict, device, sample_rate):
        ...
        with torch.no_grad():
            ...
            # side positions better
            self.model.load_state_dict(torch.load(model_dir + "/epoch-" + str(epoch) + ".model"))
            
            # front and side positions better
            # self.model.load_state_dict(torch.load(model_dir + "/sideAndFrontepoch-" + str(epoch) + ".model"))
            ...
            for vid in list_of_vids:
                string2 = vid[:-4]
                features = np.load(features_path + string2 + '_input' + '.npy')
